...
- **Inputs:** Telegram (text/voice/corrections/`/sort`/`/undo`/`/ask`), HTTP webhook,
...
- **Agent interface:** MCP read-server with search, ask (`answer_question`),
  read, catalog, siblings, schema, backlinks, recent, compartments.
...
- **Web dashboard:** stats, inbox triage (approve/reclassify/reject), compartment
  browser, recent captures (`src/inputs/dashboard.ts` + `src/views/dashboard/`).
- **Templates:** per-compartment `templates/<name>.md` with `{{vars}}` (shipped).
- **RAG:** `dendrite ask`, Telegram `/ask`, MCP `answer_question` (shipped).
- **Eval:** `dendrite eval` + `eval/dataset.jsonl` (shipped).
- **Doctor:** embedding coverage, queue health, dangling links, `--json` (shipped;
  cost estimate + avg-segments guardrail still open — §7.3).

Config already reserves `organization: folders|flat` and `tasks.render` (frontmatter
only today). Growth / MCP write blocks are being added in the polish wave below.
